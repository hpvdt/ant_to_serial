...
class serialLine:
    ...
    def sendDataBytes(self, fhr, fcad, fpow, rhr, rcad, rpow): 
        fhrb = fhr.to_bytes(1, 'big')
        fcadb = fcad.to_bytes(1, 'big')
        fpowb = fpow.to_bytes(2, 'big')
        rhrb = rhr.to_bytes(1, 'big')
        rcadb = rcad.to_bytes(1, 'big')
        rpowb = rpow.to_bytes(2, 'big')

        outputBS = fhrb + fcadb + fpowb + rhrb + rcadb + rpowb
        print(outputBS)

        # self.port.write(outputBS)
        # self.port.flush()
